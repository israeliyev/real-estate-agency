...
public class FileServiceImpl implements FileService {
    ...
    @Value("${cloud.legacy.s3.url-prefix:}")
    private String legacyS3UrlPrefix;
    ...
    public ResponseEntity<String> uploadFile(MultipartFile file) {
        ...
        try {
            ...
            PutObjectRequest putObjectRequest = PutObjectRequest.builder()
                    .bucket(bucketName)
                    .key(fileName)
                    .contentType(file.getContentType()) // ensures correct Content-Type when served
                    .build();
            ...
            String encodedKey = URLEncoder.encode(fileName, StandardCharsets.UTF_8)
                    .replace("+", "%20"); // form-encoding -> URL path encoding
            String prefix = publicUrl.endsWith("/") ? publicUrl : publicUrl + "/";
            String fileUrl = prefix + encodedKey;
    ...
    public ResponseEntity<Void> deleteFile(String filePath) {
        ...
        try {
            ...
            if (key == null) {
                log.warn("Delete request with unrecognized URL prefix: {}", filePath);
                return ResponseEntity.badRequest().build();
            }
            ...
            log.error("Failed to delete object from storage: {}", e.getMessage(), e);
    ...
    private String extractKey(String filePath) {
        ...
        if (filePath.startsWith(r2Prefix)) {
            return filePath.substring(r2Prefix.length());
        }
        if (legacyS3UrlPrefix != null && !legacyS3UrlPrefix.isBlank()) {
            String legacy = legacyS3UrlPrefix.endsWith("/") ? legacyS3UrlPrefix : legacyS3UrlPrefix + "/";
            if (filePath.startsWith(legacy)) {
                return filePath.substring(legacy.length());
            }
        }
        return null;
    }
